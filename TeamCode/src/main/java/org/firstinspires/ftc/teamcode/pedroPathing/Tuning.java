...
class LocalizationTest extends OpMode {
    static class SwyftWheel {
        public String name;
        public DcMotor motor;
        public SwyftWheel(String name, DcMotor motor) {
            this.name = name;
            this.motor = motor;
        }
    }

    private List<SwyftWheel> swyftWheels;

    @Override
    public void init() {
        this.swyftWheels = new ArrayList<>();
        SwyftDrivetrain mecanumDrivetrain = (SwyftDrivetrain)follower.getDrivetrain();
        this.swyftWheels.add(new SwyftWheel("LF", mecanumDrivetrain.getMotors().get(0)));
        this.swyftWheels.add(new SwyftWheel("LR", mecanumDrivetrain.getMotors().get(1)));
        this.swyftWheels.add(new SwyftWheel("RF", mecanumDrivetrain.getMotors().get(2)));
        this.swyftWheels.add(new SwyftWheel("RR", mecanumDrivetrain.getMotors().get(3)));
    }
    ...
    public void loop() {
        ...
        telemetryM.addData("x Ticks", follower.getPoseTracker().getLocalizer().getLateralMultiplier());
        telemetryM.addData("y Ticks", follower.getPoseTracker().getLocalizer().getForwardMultiplier());
        telemetryM.addLine("-----------");
        ...
        telemetryM.addData("X-Y hypotenuse", Math.hypot(follower.getPose().getX(), follower.getPose().getY()));

        telemetryM.addLine("----Sticks----");
        telemetryM.addData("Left Stick Y", gamepad1.left_stick_y);
        telemetryM.addData("Right Stick Y", gamepad1.left_stick_x);

        telemetryM.addLine("----Powers----");
        for (SwyftWheel wheel : this.swyftWheels) {
            telemetryM.addData(wheel.name, wheel.motor.getPower());
        }

        telemetryM.addData("MaxPowerScaling", follower.getDrivetrain().getMaxPowerScaling());
...
class ForwardZeroPowerAccelerationTuner extends OpMode {
    ...
    public void start() {
        ...
        follower.update();
        follower.setTeleOpDrive(1,0,0,true);
        follower.setMaxPowerScaling(.5); //Make sure we're going fast enough to reach target velocity.
    }
...
class LateralZeroPowerAccelerationTuner extends OpMode {
    ...
    @Override
    public void init_loop() {
        telemetryM.debug("The robot will strafe to the left until it reaches " + VELOCITY + " inches per second.");
        telemetryM.debug("Then, it will cut power from the drivetrain and roll to a stop.");
        telemetryM.debug("Make sure you have enough room.");
